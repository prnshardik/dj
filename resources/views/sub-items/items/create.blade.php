        ...
        <div class="col-md-12">
            <div class="card ">
                <div class="card-header ">
                    <h4 class="card-title">Create Sub Item</h4>
                </div>
                <div class="card-body ">
                    <form name="form" action="{{ route('sub.items.insert') }}" id="form" method="post" enctype="multipart/form-data">
                        @csrf
                                ...
                                <select name="category_id" id="category_id" class="form-control" placeholder="Plese select category">
                                    ...
                                    @if(isset($categories) && $categories->isNotEmpty())
                                        @foreach($categories as $row)
                                            <option value="{{ $row->id }}" @if(@old('category_id') == $row->id) selected @endif >{{ $row->title }}</option>
                                        @endforeach
                                    @endif
                                </select>
