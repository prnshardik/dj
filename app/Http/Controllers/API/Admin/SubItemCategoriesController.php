    ...
    class SubItemCategoriesController extends Controller{
        /** index */
            public function index(Request $request){
                $data = SubItemCategory::select('id', 'title', 'description', 'status')->get();
        ...
        /** single */
            public function single(Request $request, $id=''){
                ...
                $data = SubItemCategory::select('id', 'title', 'description', 'status')->where(['id' => $id])->first();
        ...
        /** change-status */
            public function status_change(Request $request){
                ...
                $data = SubItemCategory::where(['id' => $request->id])->first();

                if(!empty($data)){
                    if($request->status == 'deleted')
                        $update = SubItemCategory::where(['id' => $request->id])->delete();
                    else
                        $update = SubItemCategory::where(['id' => $request->id])->update(['status' => $request->status, 'updated_at' => date('Y-m-d H:i:s'), 'updated_by' => auth('sanctum')->user()->id]);
