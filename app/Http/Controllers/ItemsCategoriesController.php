    ...
    class ItemsCategoriesController extends Controller{
        /** index */
            public function index(Request $request){
                if($request->ajax()){
                    $data = ItemCategory::select('id', 'title', DB::Raw("SUBSTRING(".'description'.", 1, 30) as description"), 'status')->orderBy('id','desc')->get();

                    return Datatables::of($data)
                            ->addIndexColumn()
                            ->addColumn('action', function($data){
                                return ' <div class="btn-group btn-sm">
                                                <a href="'.route('items.categories.view', ['id' => base64_encode($data->id)]).'" class="btn btn-default btn-xs">
                                                    <i class="fa fa-eye"></i>
                                                </a> 
                                                <a href="'.route('items.categories.edit', ['id' => base64_encode($data->id)]).'" class="btn btn-default btn-xs">
        ...
        /** insert */
            public function insert(ItemsCategoryRequest $request){
                ...
                if(!empty($request->all())){
                    ...
                    if($last_id)
                        return redirect()->route('items.categories')->with('success', 'Record added successfully');
                    else
        ...
        /** view */
            public function view(Request $request, $id=''){
                ...
                $data = ItemCategory::select('id', 'title', 'description')->where(['id' => $id])->first();
                
                if($data)
                    return view('items.categories.view', ['data' => $data]);
                else
        ...
        /** edit */
            public function edit(Request $request, $id=''){
                ...
                if($data)
                    return view('items.categories.edit', ['data' => $data]);
                else
                    return redirect()->back()->with('error', 'No record found');
            }
        /** edit */ 
        ...
        /** update */
            public function update(ItemsCategoryRequest $request){
                if($request->ajax()){ return true; }

                if(!empty($request->all())){
                    $exst_record = ItemCategory::where(['id' => $request->id])->first(); 
                    ...
                    if($update)
                        return redirect()->route('items.categories')->with('success', 'Record updated successfully');
                    else
        ...
        /** change-status */
            public function change_status(Request $request){
                ...
                if(!empty($request->all())){
                    ...
                    if(!empty($data)){
                        if($status == 'deleted')
                            $update = ItemCategory::where(['id' => $id])->delete();
                        else
                            $update = ItemCategory::where(['id' => $id])->update(['status' => $status, 'updated_at' => date('Y-m-d H:i:s'), 'updated_by' => auth()->user()->id]);
